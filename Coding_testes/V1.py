...
def process_frame(seq=0, cam_id=0):
    ...
    for file in available_files:
        ...
        # Filter visible bboxes and transform to Velodyne coordinates (boxes in front of the camera)
        ...
        if masks is not None and len(masks) > 0:
            ...
            for car_idx, (car_points, color) in enumerate(zip(car_point_sets, colors)):
                if len(car_points) > 0:
                    # Find which points in points_valid correspond to this car
                    for i, point in enumerate(points_valid):
                        if not bg_assigned[i]:
                            # Check if this point is in the car_points set
                            distances = np.linalg.norm(car_points - point, axis=1)
                            if np.any(distances < 1e-6):  # Very small threshold for float comparison
                                bg_assigned[i] = True
                    ...
                    print(f"  Car {car_idx}: {len(car_points)} points")
            ...
            # Match car points to 3D bounding boxes
            print("  Matching car points to 3D bounding boxes...")
            matched_pairs = match_car_points_to_bboxes(car_point_sets, bboxes_3d, colors,
                                                       min_points=10, use_oriented=True)

            for corners, color, point_count in matched_pairs:
                bbox_lines = create_bbox_lines(corners, color=color)
                geometries.append(bbox_lines)

            print(f"  Successfully matched {len(matched_pairs)} bounding boxes")
