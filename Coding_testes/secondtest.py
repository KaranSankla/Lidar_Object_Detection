...
def is_bbox_in_camera_view(bbox_3d, camera, min_points_in_view=4, depth_range=(0.1, 100)):
    """
    Check if a 3D bounding box is visible in the camera frame

    Args:
        bbox_3d: 3D bounding box dictionary with corners_cam0
        camera: Camera object for projection
        min_points_in_view: Minimum number of corners that should be in view
        depth_range: (min_depth, max_depth) valid depth range

    Returns:
        bool: True if bbox should be kept, False if should be filtered out
        dict: Additional info about the projection
    """
    try:
        if 'corners_cam0' not in bbox_3d:
            return False, {'reason': 'no_corners'}
        ...
        valid_depth_mask = (depth >= depth_range[0]) & (depth <= depth_range[1])
        corners_with_valid_depth = np.sum(valid_depth_mask)

        if corners_with_valid_depth == 0:
            return False, {'reason': 'all_behind_camera', 'depths': depth.tolist()}

        # Check if corners are within image bounds
        valid_u_mask = (u >= 0) & (u < camera.width)
        valid_v_mask = (v >= 0) & (v < camera.height)
        valid_image_mask = valid_u_mask & valid_v_mask & valid_depth_mask

        corners_in_view = np.sum(valid_image_mask)

        # Alternative check: even if corners are outside, check if bbox occupies image space
        if corners_in_view < min_points_in_view:
            # Get 2D bounding box of all projected points (even if outside image)
            valid_depth_indices = np.where(valid_depth_mask)[0]
            if len(valid_depth_indices) > 0:
                u_valid = u[valid_depth_indices]
                v_valid = v[valid_depth_indices]

                bbox_2d_min_x, bbox_2d_max_x = np.min(u_valid), np.max(u_valid)
                bbox_2d_min_y, bbox_2d_max_y = np.min(v_valid), np.max(v_valid)

                # Check if 2D bbox intersects with image bounds
                intersects = not (bbox_2d_max_x < 0 or bbox_2d_min_x >= camera.width or
                                  bbox_2d_max_y < 0 or bbox_2d_min_y >= camera.height)

                if not intersects:
                    return False, {
                        'reason': 'no_intersection',
                        'corners_in_view': corners_in_view,
                        'bbox_2d': [bbox_2d_min_x, bbox_2d_min_y, bbox_2d_max_x, bbox_2d_max_y]
                    }

        # Additional check: filter very small projected boxes
        if corners_with_valid_depth >= 2:
            u_range = np.max(u[valid_depth_mask]) - np.min(u[valid_depth_mask])
            v_range = np.max(v[valid_depth_mask]) - np.min(v[valid_depth_mask])
            projected_area = u_range * v_range

            if projected_area < 100:  # Very small boxes (less than 10x10 pixels)
                return False, {
                    'reason': 'too_small',
        ...
        return True, {
            'reason': 'valid',
            'corners_in_view': corners_in_view,
            'corners_with_valid_depth': corners_with_valid_depth,
            'avg_depth': np.mean(depth[valid_depth_mask]) if corners_with_valid_depth > 0 else 0
        }

    except Exception as e:
        print(f"[ERROR] Error checking bbox visibility: {e}")
        return False, {'reason': 'error', 'error': str(e)}


def filter_bboxes_in_camera_view(bboxes_3d, camera, verbose=True):
    """
    Filter 3D bounding boxes to keep only those visible in camera frame

    Args:
        bboxes_3d: List of 3D bounding box dictionaries
        camera: Camera object for projection
    ...
    """
    if not bboxes_3d:
        return [], {'total': 0, 'kept': 0, 'filtered': 0, 'filter_reasons': {}}
    ...
    for i, bbox in enumerate(bboxes_3d):
        is_visible, info = is_bbox_in_camera_view(bbox, camera)

        if is_visible:
            filtered_bboxes.append(bbox)
            filter_stats['kept'] += 1
            if verbose:
                print(f"[INFO] Kept bbox {i}: {info['corners_in_view']} corners in view, "
                      f"avg depth: {info.get('avg_depth', 0):.2f}m")
        else:
            ...
            if verbose:
                print(f"[INFO] Filtered bbox {i}: {reason}")
                if reason == 'all_behind_camera':
                    depths = info.get('depths', [])
                    if depths:
                        print(f"        Depths: min={min(depths):.2f}, max={max(depths):.2f}")
                elif reason == 'no_intersection':
                    bbox_2d = info.get('bbox_2d', [])
                    print(f"        2D bbox: {bbox_2d}")
                elif reason == 'too_small':
                    print(f"        Projected area: {info.get('projected_area', 0):.1f} pixels")

    if verbose:
        print(f"\n[STATS] BBox Filtering Results:")
        print(f"        Total: {filter_stats['total']}")
        print(f"        Kept: {filter_stats['kept']}")
        print(f"        Filtered: {filter_stats['filtered']}")
        print(f"        Filter reasons: {filter_stats['filter_reasons']}")

    return filtered_bboxes, filter_stats
...
def projectVeloToImage(seq=0, cam_id=0):
    ...
    for idx, file in enumerate(available_files):
        ...
        if not bboxes_3d_raw:
            ...
            continue
        print(f"[INFO] Filtering {len(bboxes_3d_raw)} bounding boxes for camera visibility...")
        bboxes_3d_filtered, filter_stats = filter_bboxes_in_camera_view(bboxes_3d_raw, camera)

        if not bboxes_3d_filtered:
            print(f"[WARN] No bounding boxes remain after filtering for frame {frame}")
            continue
